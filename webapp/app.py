		...
		if request.method == 'POST':
			...
			with open(cwd + 'temp/'+ sourceFileName, 'rb') as f:
				...
				resp = requests.post(api_base_url + "/file_upload",
									 file=data_file, data=params, headers=headers)

			if resp.status_code == 201:
				files = requests.get(api_base_url+"/files", headers=headers)
				file_data = files.json()
				if file_data:
					return render_template('home.html', sources=file_data['files'])
	else:
		flash("not permitted this page!")
		return redirect(url_for("login"))

@app.route('/upload_convert/', methods=['GET', 'POST'])
@app.route('/upload_convert', methods=['GET', 'POST'])
def upload_convert():
	if session.get("token") is None:
		if request.method == 'GET':
		...
		if request.method == 'POST':
			...
			password = request.form.get('targetType')

			params = {"file":file, "targetType":targetType}
			headers = {"Content-type": "application/json"}
			resp = requests.post(api_base_url + "/file_upload/",
									 data=json.dumps(params), headers=headers)
